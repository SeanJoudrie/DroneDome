...
/**
 * Does this aircraft arrive with this part fitted?
 *
 * Three kinds of evidence, because a part can be real without being its own
 * mesh: a classified node, a cut that defines the region to clip out of a
 * welded hull, or — for a wing — a published wing area. The Cessna's wing is
 * welded into its fuselage so no wing mesh exists, and it certainly has one.
 *
 * This lived in three places that had drifted apart: the stats panel, the slot
 * label and the part drawer. When they disagree the label says "not fitted"
 * over a control that works, or offers a control for a part that is not there.
 */
export function hasStockRole(base: (typeof AIRCRAFT)[number] | undefined, role: PartRole): boolean {
...
/**
 * What the app believes can be changed, published for the capability check.
 *
 * The check used to read a file I generated by hand once and left in /tmp, so
 * it could not run from a clean checkout and it described whatever the catalog
 * looked like the day I wrote it. Asking the app is the same lesson the
 * geometry check already learned: an audit that reimplements the thing it is
 * auditing eventually audits its own copy.
 */
export function capabilityMatrix() {
  const donors: Record<string, string[]> = {}
  const cross: Record<string, [string, PartRole][]> = {}
  for (const role of SWAPPABLE_ROLES) {
    donors[role] = donorsFor(role).map((a) => a.id)
    cross[role] = crossRolePartsFor(role).map((c) => [c.aircraft.id, c.fromRole])
  }
  return {
    roles: SWAPPABLE_ROLES,
...
export function slotLabel(build: Build, role: PartRole): string {
  ...
  if (choice.kind === 'stock') {
    ...
    return hasStockRole(base, role) ? 'Standard' : 'Not fitted'
  }
